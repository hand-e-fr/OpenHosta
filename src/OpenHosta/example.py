    ...
    **kwargs
    ...
    if args != ():
        if len(args) != len(input_type):
            raise ValueError(f"Too many arguments for function {func.__name__}, please provide {len(input_type)} arguments, use hosta_out for output")
        ...
        for key, value in kwargs.items():
            ...
            if not isinstance(value, expected_type):
                raise ValueError(f"Argument {value} does NOT match the expected type {expected_type} for parameter {key}. For function {func.__name__}")
...
def save_examples(hosta_func=None, hosta_path=None):
    ...
    if hosta_path is None:
        raise ValueError(f"Please provide hosta_path for specifying the path to save the cache")
...
def load_examples(hosta_func=None, hosta_path=None):
    ...
    if hosta_path is None:
        raise ValueError(f"Please provide hosta_path for specifying the path to load the cache")
