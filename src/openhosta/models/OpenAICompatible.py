...
class OpenAICompatibleModel(Model):
    ...
    def __init__(self,
        ...
        if capabilities is None:
            capabilities = {ModelCapabilities.TEXT2TEXT, ModelCapabilities.STREAMING}
        ...
        if any(var is None for var in (model_name, base_url)):
            raise ValueError("[Model.__init__] Missing values.")
    ...
    def get_thinking_and_data_sections(
                        self,
                        response:str,
                        reasoning_start_and_stop_tags = None) -> tuple[str, str]:
        """
        ...
        """
        if reasoning_start_and_stop_tags is None:
            reasoning_start_and_stop_tags = ["<think>", "</think>"]
        rational = ""
        answer = ""
    ...
    def print_last_prompt(self, inspection):
        ...
        if "llm_api_messages_sent" in inspection.logs:
            for _i, msg in enumerate(inspection.logs["llm_api_messages_sent"]):
