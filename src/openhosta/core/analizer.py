...
def nice_type_name(p_type) -> str:
    ...
    if hasattr(p_type, "__origin__"):
        from openhosta.guarded.primitives import Guarded as GuardedPrimitiveMarker
        from openhosta.guarded.wrapper import Guarded as GuardedWrapper
...
def _resolve_annotation(annotation, globalns=None, localns=None):
    ...
    try:
        ...
        warnings.warn(f"[OpenHosta] Could not resolve stringified annotation '{annotation}'. Falling back to Any.", stacklevel=2)
        return typing.Any
...
def hosta_analyze(frame=None, function_pointer=None) -> AnalyzedFunction:
    try:
        ...
        import warnings
        warnings.warn(f"[OpenHosta] Unexpected error resolving type hints for {getattr(function_pointer, '__name__', '?')}: {e}", stacklevel=2)
        hints = {}
...
def encode_function(analyse: AnalyzedFunction, model_capability=None):
    """
    Encode the function signature and docstring into a format suitable for the model.
    """
    if model_capability is None:
        model_capability = set([ModelCapabilities.TEXT2TEXT])
    snippets  = {}
...
def encode_function_parameter_values(analyse: AnalyzedFunction):
    ...
    for a in args:
        call_arg, value, _arg_type = a.name, a.value, a.type
