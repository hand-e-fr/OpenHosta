...
class Model:
    ...
    def api_call(self, messages: list[dict[str, str]], llm_args: dict = None) -> dict:
        if llm_args is None:
            llm_args = {}
        return self.generate(messages, **llm_args)

    async def api_call_async(self, messages: list[dict[str, str]], llm_args: dict = None) -> dict:
        if llm_args is None:
            llm_args = {}
        return await self.generate_async(messages, **llm_args)

    def api_call_without_retry(self, messages: list[dict[str, str]], llm_args: dict = None) -> dict:
        if llm_args is None:
            llm_args = {}
