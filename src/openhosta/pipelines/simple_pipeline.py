...
from ..core.meta_prompt import EMULATE_META_PROMPT, USER_CALL_META_PROMPT, MetaPrompt
from ..core.uncertainty import (
    ReproducibleSettings,
    get_certainty,
    get_enum_logprobes,
    normalized_probs,
    reproducible_settings_ctxvar,
)
from ..guarded.primitives import Guarded, GuardedPrimitive, ProxyWrapper
...
class OneTurnConversationPipeline(Pipeline):
    ...
    def push_detect_missing_types(self, inspection:Inspection):
        ...
        if inspection.analyse.type is None:
            warnings.warn(f"No return type found for function {inspection.analyse.name}. Assuming str.", stacklevel=2)
            inspection.analyse.type = str

        # Check each argument type
        for arg in inspection.analyse.args:
            if arg.type is None:
                warnings.warn(f"No type found for argument {arg.name}. Assuming str.", stacklevel=2)
                arg.type = str
    ...
    def pull_check_uncertainty(self, inspection:Inspection) -> Any:
        ...
        hash( str(inspection.logs.get("llm_api_messages_sent", "")) )
    ...
    def push(self, inspection:Inspection) -> dict:
        ...
        self.push_choose_model(inspection)
