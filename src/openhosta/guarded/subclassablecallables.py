...
from .defaults import ALLOW_CODE_EXECUTION
...
class GuardedCode(GuardedPrimitive, ProxyWrapper):
    ...
    def _parse_heuristic(cls, value: Any) -> tuple[UncertaintyLevel, Any, str | None]:
        ...
        if not ALLOW_CODE_EXECUTION:
            raise RuntimeError("Code execution disabled")

        try:
            # Scope isolé par invocation pour éviter fuites entre appels
            local_scope: dict[str, Any] = dict(
                _SAFE_BUILTINS, **cls._type_knowledge.get("local_scope", {})
            )
