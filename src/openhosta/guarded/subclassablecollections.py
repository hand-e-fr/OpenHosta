...
from .primitives import GuardedCallInput, GuardedPrimitive, ProxyWrapper, Tolerance, UncertaintyLevel
from .type_hints import resolve_struct_hints

_GUARDED_LIST_CACHE: weakref.WeakKeyDictionary[Any, Any] = weakref.WeakKeyDictionary()
_GUARDED_SET_CACHE: weakref.WeakKeyDictionary[Any, Any] = weakref.WeakKeyDictionary()
_GUARDED_DICT_CACHE: dict[tuple, Any] = {}
...
class GuardedList(GuardedPrimitive, list):
    ...
    def __class_getitem__(cls, item):
        ...
        try:
            cache[item] = ParameterizedGuardedList
        except TypeError:
            pass
...
class GuardedSet(GuardedPrimitive, set):
    ...
    def __class_getitem__(cls, item):
        ...
        try:
            cache[item] = ParameterizedGuardedSet
        except TypeError:
            pass
...
class GuardedDict(GuardedPrimitive, dict):
    ...
    def __class_getitem__(cls, item):
        if not isinstance(item, tuple) or len(item) != 2:
            return cls
        if item in _GUARDED_DICT_CACHE:
            return _GUARDED_DICT_CACHE[item]
        ...
        _GUARDED_DICT_CACHE[item] = ParameterizedGuardedDict
    ...
    def decorator(cls_to_guard):
        ...
        class GuardedDataclassWrapper(GuardedPrimitive, ProxyWrapper):
            ...
            def _coerce_dataclass_inputs(cls, args_tuple, kwargs_dict):
                ...
                for field_name, arg_value in zip(field_names, args_tuple, strict=False):
                    ...
                    try:
                        ...
                        if isinstance(tree.body, ast.Call):
                            ...
                            for kw in tree.body.keywords:
                                try:
                                    parsed_kwargs[kw.arg] = ast.literal_eval(kw.value)
                                except Exception:
                                    parsed_kwargs[kw.arg] = ast.unparse(kw.value)
