...
def guarded_pydantic_model(model_cls: type[BaseModel]) -> type[GuardedPrimitive]:
    ...
    class GuardedPydanticModel(GuardedPrimitive, ProxyWrapper):
        ...
        _type_json = model_cls.model_json_schema() if hasattr(model_cls, 'model_json_schema') else model_cls.schema()
        ...
        def _parse_heuristic(cls, value: Any):
            ...
            try:
                ...
                # On itère sur les champs du modèle (Pydantic v1/v2)
                fields_dict = getattr(model_cls, 'model_fields', getattr(model_cls, '__fields__', {}))
                for field_name, field_info in fields_dict.items():
                    if field_name not in data_dict:
                        continue

                    raw_val = data_dict[field_name]
                    expected_type = hints.get(field_name, getattr(field_info, 'annotation', getattr(field_info, 'type', None)))
