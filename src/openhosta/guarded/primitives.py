...
@dataclass(frozen=True)
class GuardConfig:
    """Configuration déclarative pour un type Guarded via typing.Annotated.

    Attributes:
        tolerance: Seuil d'incertitude maximal (0.0=strict, 1.0=anything).
        semantic_fallback: Stratégie de fallback sémantique, ex. "llm".
        retries: Nombre de tentatives de cast avant abandon.
    """
...
class CastingResult:
    ...
    original_input: Any
    guarded_data: Any = None # Instance de GuardedPrimitive ou ProxyWrapper correspondant au type attendu
    python_type: type | None = None

    error_message: str | None = None

    @classmethod
    def ok(cls, value: Any, confidence: float = 1.0, abstraction: AbstractionLevel = "native") -> "CastingResult":
        """Create a successful CastingResult with keyword arguments."""
        return cls(
            success=True,
            data=value,
    ...
    def failure(
        ...
        error_message: str | None = None,
        abstraction: AbstractionLevel = "failed",
    ) -> "CastingResult":
        """Create a failed CastingResult with keyword arguments."""
        return cls(
            success=False,
            data=None,
            uncertainty=1.0 - confidence,
...
class GuardedPrimitive(ABC, metaclass=GuardedPrimitiveMeta):
    ...
    @staticmethod
    def _find_first_out_of_string_colon(s: str) -> int | None:
        """
    ...
    def attempt(cls, value: Any, tolerance: ToleranceLevel|None = None) -> CastingResult:
            ...
            try:
                ...
                if isinstance(instance, (list, set, dict)):
                    ...
                    if isinstance(instance, list):
                        instance.extend(cleaned_val)
                    elif isinstance(instance, set):
                        instance.update(cleaned_val)
        ...
        return CastingResult(
            success=False,
            data=None,
            uncertainty=Tolerance.ANYTHING,
            abstraction="failed",
            original_input=value,
            python_type=cls._type_py,
    ...
    @classmethod
    def _parse_knowledge(cls, value: Any) -> tuple[UncertaintyLevel, Any, str | None]:
        """
...
class ProxyWrapper:
    ...
    _hosta_inspection: Any | None = None
    ...
    def _unwrap(value: Any) -> Any:
        """Dé-proxyer une valeur : si c'est un ProxyWrapper retourne _python_value."""
        if isinstance(value, ProxyWrapper):
            return getattr(value, "_python_value", value)
        return value
    ...
    # --- Arithmetic dunder delegation ---
    def __add__(self, other):
        return self._python_value + self._unwrap(other)
    ...
    def __int__(self):
        return int(self._python_value)

    def __float__(self):
        return float(self._python_value)

    def __bool__(self):
        return bool(self._python_value)

    def __complex__(self):
        return complex(self._python_value)
    ...
    # --- Negation / abs dunder delegation ---
    def __neg__(self):
