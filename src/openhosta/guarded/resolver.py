...
# Imports internes des primitives OpenHosta
from .primitives import CastingResult, GuardConfig, Guarded, GuardedPrimitive


class SemanticBackend(Protocol):
    """Interface pour un moteur de résolution sémantique injectable."""

    def resolve(self, value: str, target_type: type) -> CastingResult: ...


_SEMANTIC_BACKEND: SemanticBackend | None = None


def register_semantic_backend(backend: SemanticBackend) -> None:
    """Enregistrer un backend sémantique global pour les fallbacks LLM."""
    global _SEMANTIC_BACKEND
    _SEMANTIC_BACKEND = backend

from .subclassablecallables import GuardedCode
from .subclassablecollections import (
    GuardedDict,
    GuardedList,
    GuardedSet,
    GuardedTuple,
    ...
    def _do_resolve(cls, annotation: Any) -> type[GuardedPrimitive]:
        ...
        if origin is not None:
            ...
            if origin in (Callable, typing.Callable, collections.abc.Callable):
                from .subclassablecallables import GuardedCode, guarded_callable
            ...
            if origin in (set, frozenset, set, typing.AbstractSet, collections.abc.Set):
                if args and len(args) != 1:
                    raise TypeError(
                        f"Invalid type annotation: {annotation!r}. "
            ...
            # Annotated: extract GuardConfig and override _tolerance on the resolved class
            if origin is typing.Annotated:
                base_type = cls.resolve(args[0])
                gc = None
                for meta in args[1:]:
                    if isinstance(meta, GuardConfig):
                        gc = meta
                        break
                if gc is not None:
                    # Dynamically subclass to apply the GuardConfig overrides
                    config_attrs: dict[str, Any] = {"_tolerance": gc.tolerance}
                    if gc.semantic_fallback == "llm" and _SEMANTIC_BACKEND is not None:
                        backend_ref = _SEMANTIC_BACKEND
                        target = getattr(base_type, "_type_py", object)
                        @classmethod
                        def _parse_semantic_cb(
                            _cls: type[GuardedPrimitive], value: Any
                        ) -> tuple[Any, Any, Any]:
                            from .primitives import Tolerance, UncertaintyLevel
                            cr = backend_ref.resolve(str(value), target)
                            if cr.success:
                                return UncertaintyLevel(0.15), cr.data, None
                            return UncertaintyLevel(Tolerance.ANYTHING), value, cr.error_message

                        config_attrs["_parse_semantic"] = _parse_semantic_cb

                    dynamic_cls = type(
                        f"{base_type.__name__}_cfg",
                        (base_type,),
                        config_attrs,
                    )
                    return dynamic_cls
