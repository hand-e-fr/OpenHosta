...
class Tolerance:
    ...
    TYPE_COMPLIANT:ToleranceLevel = 1.0 - 1e-15

    # --- 6. ERREUR DE TYPE (Le "Chaos" absolu) ---
    ANYTHING:ToleranceLevel = 1.0

    @staticmethod
    def describe(value: ToleranceLevel) -> str:
        """Retourne une description textuelle pour le debug."""
        if value <= Tolerance.STRICT:
            return "STRICT (Identité)"
        if value <= Tolerance.PRECISE:
            return "PRECISE (Logique)"
        if value <= Tolerance.FLEXIBLE:
            return "FLEXIBLE (Intention)"
        if value <= Tolerance.CREATIVE:
            return "CREATIVE (Thématique)"
        if value == Tolerance.ANYTHING:
            return "ANYTHING (Chaos)"
